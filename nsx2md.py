...
insert_title = False  # True will add the title of the note as a field in the YAML block, False no title in block.
...
media_dir_name = 'assets'  # name of the directory inside the produced directory where all images and attachments will be stored
...
print('work path {}'.format(work_path))
print('pandoc exe file {}'.format(pandoc_run_file))

try:
    pandoc_ver = subprocess.check_output([pandoc_run_file, '-v'], timeout=3).decode('utf-8')[7:].split('\n', 1)[0].strip()
    print('Found pandoc ' + pandoc_ver)

    if distutils.version.LooseVersion(pandoc_ver) < distutils.version.LooseVersion('1.16'):
        pandoc_args = [pandoc_run_file, '-f', 'html', '-t', 'markdown_strict+pipe_tables-raw_html',
...
for file in files_to_convert:
    ...
    for notebook_id in config_data['notebook']:
        ...
        # Richard, Siyuan prefer single assets dir instead of multiple assets dir in each notebook.
        # notebook_media_path = Path(notebook_path / media_dir_name)
        notebook_media_path = Path(work_path / media_dir_name)
        
        notebook_path.mkdir(parents=True)
        
        if os.path.exists(notebook_media_path) == False:
    ...
    for note_id in config_data['note']:
        ...
        text_ctime = time.strftime('%Y-%m-%d %H:%M', time.localtime(note_ctime))
        text_mtime = time.strftime('%Y-%m-%d %H:%M', time.localtime(note_mtime))
        content = '`Imported from Synology - Note Station. Created @{}. Modified @{}.` \n\n '.format(text_ctime, text_mtime) + content


        block = ''
        
        if meta_data_in_yaml:
            block = create_yaml_meta_block()
        else:
            block = create_text_meta_block()
